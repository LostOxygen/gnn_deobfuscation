...
class Attention(nn.Module):
    ...
    def forward(self, nodes, edges, mask=None):
        ...
        q, k, v, e_kv = map(lambda t: rearrange(
            t, 'b ... (h d) -> (b h) ... d', h=h), (q, k, v, e_kv))

        if exists(self.pos_emb):
            freqs = self.pos_emb(torch.arange(
                nodes.shape[1], device=nodes.device))
        ...
        if exists(mask):
            mask = rearrange(
                mask, 'b i -> b i ()') & rearrange(mask, 'b j -> b () j')
    ...
    def __init__(
        ...
        for _ in range(depth):
            self.layers.append(List([
                List([
                    PreNorm(dim, Attention(dim, pos_emb=pos_emb,
                            edge_dim=edge_dim, dim_head=dim_head, heads=heads)),
                    GatedResidual(dim)
                ]),
                List([
